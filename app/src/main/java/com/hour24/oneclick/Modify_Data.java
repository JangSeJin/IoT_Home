...
public class Modify_Data {
	...
	public Modify_Data(Activity activity) {
		super();
	...
	public void onModify(JSONObject jsonOneclick, String originalData) {
		...
		if (jsonData.equals("") || jsonData.equals("false")) {
			...
			try {
				...
				Log.e("devIoT", "수정하기전  : " + jsonMain.toString());
				Log.e("devIoT", "originalData : " + originalData);

				// Log.e("devIoT", newName + " / " + oldName);
				// originalData = originalData.replaceAll(newName.trim(),
				// oldName.trim());
				// Log.e("devIoT", "이름 바꾼데이터 : " + originalData);
				originalData = ",{\"OneClickElement\":" + originalData + "}";
				String sub = jsonMain.toString().replace(originalData, "");
				Log.e("devIoT", "자른 데이터 : " + sub);

				new MySharedPreferences(activity).clear("OneClick");
				new MySharedPreferences(activity).set("OneClick", sub);
				jsonParse(sub);
